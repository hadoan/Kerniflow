...
async function main() {
  ...
  for (const register of registers) {
    ...
    for (let i = 0; i < register.dayCloses.length; i++) {
      ...
      const dayEntries = register.entries.filter(e => e.dayKey === dayClose.dayKey && e.status !== 'CANCELLED' && e.status !== 'DRAFT' && e.type !== 'INTERNAL_TRANSFER' && e.type !== 'LOCATION_TRANSFER');
      
      let inflows = 0;
      let outflows = 0;

      for (const e of dayEntries) {
        if (e.direction === "IN") inflows += e.amountCents;
        if (e.direction === "OUT") outflows += e.amountCents;
      }
      ...
      if (dayClose.expectedBalanceCents !== expectedClosing) {
        console.log(`Updating expected balance for ${dayClose.dayKey}: ${dayClose.expectedBalanceCents} -> ${expectedClosing}`);
      }
      ...
      if (!isDryRun) {
        await prisma.cashDayClose.update({
          ...
          data: {
            ...
            verificationStatus: counted === null ? "NOT_COUNTED" : (counted === expectedClosing ? "COUNTED_MATCH" : "COUNTED_DIFFERENCE")
          }
        });
      }
