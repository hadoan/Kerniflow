...
import { extractReceiptData, createExpense, generateInvoiceDraftFromPrompt, issueInvoice } from '@/shared/mock/mockApi';
import { getDb } from '@/shared/mock/mockDb';
import type { ExtractReceiptResult, CreateExpenseInput, ExpenseCategory, VatRate, Invoice } from '@/shared/types';
...
export default function AssistantPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasFile || userMessage.toLowerCase().includes('receipt') || userMessage.toLowerCase().includes('expense') || userMessage.toLowerCase().includes('beleg') || userMessage.toLowerCase().includes('ausgabe')) {
        await handleReceiptFlow(userMessage);
      }
      // Check for invoice flow
      else if (userMessage.toLowerCase().includes('invoice') || userMessage.toLowerCase().includes('rechnung')) {
        await handleInvoiceFlow(userMessage);
      }
      // Check for tax question
      else if (userMessage.toLowerCase().includes('deduct') || userMessage.toLowerCase().includes('tax') || userMessage.toLowerCase().includes('absetzen') || userMessage.toLowerCase().includes('steuer')) {
  ...
  const handleReceiptFlow = async (userMessage: string) => {
  ...
  const handleTaxQuestion = async () => {
    ...
    const response = i18n.language === 'de'
...
⚠️ ${t('assistant.disclaimer')}`;
  ...
  const handleSuggestionClick = (key: string) => {
    const suggestionTexts: Record<string, { de: string; en: string }> = {
      addReceipt: { de: 'Diesen Beleg als Ausgabe hinzufügen', en: 'Add this receipt as an expense' },
      generateInvoice: { de: 'Rechnung für Müller GmbH erstellen', en: 'Generate invoice for Müller GmbH' },
              ...
              >
                ...
                <div className={cn(
                  ...
                  {message.attachments?.map((att, i) => (
                    <div key={i} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-muted text-sm">
                    ...
                    <Card className={cn(
                      ...
                      <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium">
                          {t(`assistant.toolCards.${
                            message.toolCall.name === 'extract_receipt_data' ? 'extractingReceipt' :
                            message.toolCall.name === 'create_expense' ? 'creatingExpense' :
                            message.toolCall.name === 'generate_invoice_draft' ? 'generatingInvoice' :
                            'issuingInvoice'
                          }`)}
                        </span>
                      </CardContent>
                      ...
                      <CardContent className="p-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-medium">{formatMoney(message.expenseForm.totalCents, locale)}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">{t('expenses.date')}</div>
                            <div className="font-medium">{formatDate(message.expenseForm.date, locale)}</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">{t('expenses.category')}</div>
                            <div className="font-medium">{t(`expenses.categories.${message.expenseForm.suggestedCategory}`)}</div>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <div className="font-medium">{Math.round(message.expenseForm.confidence * 100)}%</div>
                          </div>
                        </div>
                      ...
                      <CardContent className="p-4 space-y-4">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{message.invoicePreview.invoiceNumber}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">{t('invoices.client')}</span>
                            <span className="font-medium">
                              {db.clients.find(c => c.id === message.invoicePreview?.clientId)?.company}
                            </span>
                          </div>
                          ...
                          <div className="border-t border-border pt-2">
                            <div className="flex justify-between">
                              ...
                              <span>{formatMoney(message.invoicePreview.subtotalCents, locale)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t('invoices.vatAmount')}</span>
                              <span>{formatMoney(message.invoicePreview.vatAmountCents, locale)}</span>
                            </div>
