...
test.describe("Daily Cash E2E - Business Bank Deposit Regression", () => {
  ...
  test("400€ deposit to business bank should ask for money source, not payment method", async ({
    ...
    await expect(assistant.page.getByRole("button", { name: /Gehört noch zum aktuellen Kassenbestand|Still belongs to current register balance/i })).toBeVisible();
    await expect(assistant.page.getByRole("button", { name: /Wurde bereits als Entnahme gebucht|Already recorded as taken out/i })).toBeVisible();
    await expect(assistant.page.getByRole("button", { name: /Privates Geld|Private money/i })).toBeVisible();
