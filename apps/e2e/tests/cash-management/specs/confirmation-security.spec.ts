...
test.describe("Daily Cash E2E - Confirmation Security", () => {
  ...
  test("Case 12 & 13: No writes before explicit confirmation", async ({ cashContext, cashPage }) => {
    ...
    let { entries } = await listCashEntries(cashContext.client.httpClient, cashContext.register.id, { dayKey });
    ...
    let dbResult = await listCashEntries(cashContext.client.httpClient, cashContext.register.id, { dayKey });
